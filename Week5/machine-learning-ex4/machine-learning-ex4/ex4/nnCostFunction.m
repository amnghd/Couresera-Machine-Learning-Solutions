...
Ynot=1-Y;
A2not=1-A2;
reg_item=lambda/(2*m)*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
J=-1/m*sum(sum(Y.*log(A2)+Ynot.*log(A2not)))+reg_item;
...
a2=A1;
z2=(X*Theta1');
a3=sigmoid(A1*Theta2');
z3=(A1*Theta2');
del3=a3-Y;
del2=del3*Theta2(:,2:end).*sigmoidGradient(z2);
Delta1=zeros(size(del2'*a1));
Delta2=zeros(size(del3'*a2));
